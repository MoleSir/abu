...
async fn result_main() -> anyhow::Result<()> {    
    ...
    let (cron_tx, cron_rx) = mpsc::channel::<String>(100);
    let cron_scheduler = Arc::new(Mutex::new(CronScheduler::new("./")?));
    cron_scheduler.lock().await.start(cron_tx).await?;
    let res = agent_main(cron_scheduler.clone(), cron_rx).await;
    cron_scheduler.lock().await.stop().await;
    res
}

async fn agent_main(cron_scheduler: Arc<Mutex<CronScheduler>>, mut cron_rx: mpsc::Receiver<String>) -> anyhow::Result<()> {
    ...
    loop {
        ...
        tokio::select! {
            // 分支 A: 等待用户在终端输入
            res = reader.read_line(&mut query) => {
                res?;
                let trimmed_query = query.trim();
                
                if trimmed_query == "q" || trimmed_query == "quit" || trimmed_query.is_empty() {
                    break;
                }        
                
                agent.run(trimmed_query).await?;
                query.clear();
            }

            // 分支 B: 等待 Cron 后台线程发来消息
            Some(cron_notification) = cron_rx.recv() => {
                // 当 Cron 触发时，打断当前等待，直接让 Agent 执行
                println!("\n[System] Cron task triggered!");
                
                let system_query = format!("[Cron notification] {}", cron_notification);
                agent.run(&system_query).await?;
            }
        }
    }
...
impl CronScheduler {
    ...
    pub async fn start(&mut self, tx: mpsc::Sender<String>) -> anyhow::Result<()> {
        // 加载文件中的任务
        ...
        tasks.retain_mut(|task| {
            ...
            if cron_matches(&task.cron_expr, &check_time) {
                ...
                if let Err(e) = tx.try_send(notification) {
                    eprintln!("[Cron] Failed to send notification to channel: {}", e);
                }
